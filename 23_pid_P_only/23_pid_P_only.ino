...
#define _DUTY_NEU 1490  // Servo angle: 0 degree
...
void loop()
{
  ...
  if(event_dist) {
    ...
    dist_filtered = volt_to_distance(ir_sensor_filtered(5, 0.5));
...
//    int rad = 0;
//    if (duty_curr > _DUTY_NEU){
//      rad = (-1) * (float)(duty_curr - _DUTY_NEU) / (_DUTY_MAX - _DUTY_NEU) * LIMIT_RAD;
//    } else {
//      rad = (float)(duty_curr - _DUTY_NEU) / (_DUTY_MIN - _DUTY_NEU) * LIMIT_RAD;
//    }
//    if (rad > LIMIT_RAD) {
//      rad = LIMIT_RAD;
//    } else if (rad < (-1) * LIMIT_RAD) {
//      rad = (-1) * LIMIT_RAD;
//    }
//    control = pterm / cos(radians(rad));
    control = pterm;
...
{
  return (644.0 + (-2.23 * a_value) + (0.00192 * a_value * a_value));
}
