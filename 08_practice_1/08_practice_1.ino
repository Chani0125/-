...
float pre_distance = _DIST_MIN - 10.0;
int brightness = 0;
...
void loop() {
  ...
  if (distance < _DIST_MIN) {
    distance = pre_distance;    // Set Lower Value
  } else if (distance > _DIST_MAX) {
    distance = pre_distance;    // Set Higher Value
  ...
  bright = (int)((1-(1-(float)bright / 100)*(1-(float)bright / 100)) * 255);
